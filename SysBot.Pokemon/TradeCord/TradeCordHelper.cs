    ...
    private Results CatchHandler(TCUser user, int format = 9)
    {
        ...
        bool FuncCatch()
        {
            ...
            if (Rng.CatchRNG >= 100 - Settings.CatchRate)
            {
                ...
                if (result.Poke.Species is ((ushort)Species.Hoopa) or ((ushort)Species.Keldeo) or ((ushort)Species.Meloetta) or ((ushort)Species.Koraidon) or ((ushort)Species.Miraidon))
                    result.Poke.Form = 0;
